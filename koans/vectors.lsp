...
(defun list-to-bit-vector (my-list)
  (let ((my-list-length (length my-list))
        (my-bit-vector))
    (setf my-bit-vector (make-array my-list-length :element-type 'bit))
    (dotimes (i my-list-length)
      (if (= 1 (nth i my-list))
        (setf (row-major-aref my-bit-vector i) 1)
        (setf (row-major-aref my-bit-vector i) 0)))
    my-bit-vector))
